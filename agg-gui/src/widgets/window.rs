...
impl Window {
    ...
    /// partially off-screen, regardless of where its host places it in the
    /// tree. Called from `paint_global_overlay`, where the ctx's root transform
    /// carries the accumulated ancestor offset (the overlay walk applies only
    /// translations, no clips). Returns the logical `(dx, dy)` correction so the
    /// caller can shift the paint by the same amount this frame; the correction
    /// is also folded into `bounds` so the next frame's input routing (which
    /// reads `bounds` in the same parent-local space) stays in lockstep.
    ///
    /// Coordinates are logical Y-up with the origin at the viewport's
    /// bottom-left — the window's local `(0, 0)` is its own bottom-left corner,
    /// so a fit requires `0 ≤ root_y` and `root_y + height ≤ viewport.height`.
    fn clamp_modal_into_viewport(&mut self, ctx: &dyn DrawCtx) -> (f64, f64) {
        let vp = crate::widget::current_viewport();
        if vp.width <= 1.0 || vp.height <= 1.0 {
            return (0.0, 0.0);
        }
        // `App::paint` scales the ctx by the combined device × UX zoom factor,
        // so the root transform maps logical units up by `effective_scale`;
        // divide by the same to recover logical (device-independent) root
        // coordinates that match `current_viewport()`.
        ...
        let root_x = ox / scale;
        let root_y = oy / scale;
        let max_x = (vp.width - self.bounds.width).max(0.0);
        let max_y = (vp.height - self.bounds.height).max(0.0);
