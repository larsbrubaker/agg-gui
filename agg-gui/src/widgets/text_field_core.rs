...
pub fn word_range_at(s: &str, byte_pos: usize) -> (usize, usize) {
    ...
    let start = {
        ...
        while p > 0 {
            ...
            if is_word_char(c) != anchor_class { break; }
            p = prev;
        }
        p
    };
...
pub fn byte_at_x(font: &Font, text: &str, font_size: f64, target_x: f64) -> usize {
    ...
    for (i, c) in text.char_indices() {
        ...
        if target_x < mid { return prev_pos; }
        prev_x   = x;
        prev_pos = i;
        let _ = c;
    }
    ...
    if target_x < mid { prev_pos } else { text.len() }
}
...
impl UndoRedoCommand for TextEditCommand {
    ...
    fn do_it(&mut self)   { *self.target.borrow_mut() = self.after.clone(); }
    fn undo_it(&mut self) { *self.target.borrow_mut() = self.before.clone(); }
}
