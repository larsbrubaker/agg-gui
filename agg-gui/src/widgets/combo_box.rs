...
use super::scrollbar::{
    PreparedScrollbar, ScrollbarAxis, ScrollbarGeometry, ScrollbarOrientation, DEFAULT_GRAB_MARGIN,
};
...
fn submit_combo_popup_internal(request: ComboPopupRequest) {
    COMBO_POPUP_QUEUE.with(|q| q.borrow_mut().push(request));
}

mod popup_paint;
pub(crate) use popup_paint::{begin_combo_popup_frame, paint_global_combo_popups};
pub(super) use popup_paint::{current_combo_viewport, submit_combo_popup};
