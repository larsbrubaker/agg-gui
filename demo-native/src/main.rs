...
//! (winit + wgpu surface), the event loop, and per-frame plumbing.  It
//! contains **no demo content**: every widget tree, layout, and GPU renderer
//! the user sees is shared via `demo-wgpu` (the wgpu rendering library) and
//! `demo-ui` (the widget tree + layout).
//!
...
//! - **Platform shell (OS window, event loop)** → here and `demo-wasm`
//!
//! # Scope
//!
...
//! Future: state persistence, fullscreen toggle, screenshot capture, MSAA
//! selection, touch events, hi-DPI scale tracking — all to be ported from the
//! pre-migration GL shell once visually validated.
