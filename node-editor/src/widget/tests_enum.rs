...
/// A node carrying one enum property named `operation`.
fn mk_enum_node(current: &str) -> crate::model::NodeView {
...
fn editor_with_enum_node(current: &str) -> (NodeEditor, Arc<Mutex<super::tests_common::Memory>>) {
    ...
    seed_nodes(&mut editor, &memory, vec![mk_enum_node(current)]);
    (editor, memory)
}
...
fn paint_enum_node(current: &str) -> StripRecorder {
    let (mut editor, _memory) = editor_with_enum_node(current);
