...
| Modals | `demo-ui/src/windows/text_demos/dialogs.rs` | `egui-reference/crates/egui_demo_lib/src/demo/modals.rs` | needs_test | Reworked into stacked modal layers with backdrop, Escape/outside-click close, progress modal, app-viewport overlay painting, editable user name, and role ComboBox. Remaining gap: exact egui `Modal` API surface. |
...
| Strip | `demo-ui/src/windows/text_demos/strip_table.rs` | `egui-reference/crates/egui_demo_lib/src/demo/strip_demo.rs` | needs_test | Reworked to egui's nested exact/remainder/relative strip regions and now enforces exact/at-least lower bounds for the fixed rects. |
...
- `cargo test -p demo-ui modal_ -- --nocapture` passed after making User modal fields functional.
- `cargo test -p demo-ui strip_regions -- --nocapture` passed after adding Strip lower-bound coverage.
