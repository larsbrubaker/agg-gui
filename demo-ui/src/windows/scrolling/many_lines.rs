...
use agg_gui::{FlexColumn, FlexRow, Font, Rect, ScrollView, Separator, SizedBox, Slider, Widget};
...
pub fn build(font: Arc<Font>) -> Box<dyn Widget> {
    ...
    let scroll = ScrollView::new(Box::new(list)).with_viewport_cell(Rc::clone(&viewport));
