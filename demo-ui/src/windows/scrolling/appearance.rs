...
use agg_gui::{
    current_scroll_style, set_scroll_style, Checkbox, CollapsingHeader, DragValue, FlexColumn,
    FlexRow, Font, Insets, Label, Rect, ScrollBarColor, ScrollBarKind, ScrollBarStyle,
...
fn drag_row(
    ...
    Box::new(
        FlexRow::new()
            .with_gap(10.0)
            .add(Box::new(
                SizedBox::new().with_width(CTRL_W).with_child(Box::new(dv)),
            ))
...
pub fn build(font: Arc<Font>) -> Box<dyn Widget> {
    ...
    let presets_row: Box<dyn Widget> = Box::new(
        FlexRow::new()
            .with_gap(10.0)
    ...
    // Row 1: Type
    {
        ...
        let seg = SegRow::new(
            Arc::clone(&font),
            vec![
                ("Solid", ScrollBarKind::Solid),
                ("Floating", ScrollBarKind::Floating),
            ],
            Rc::clone(&kind_cell),
        )
        .on_change(move || {
    ...
    );
    {
        ...
        let row = drag_row(
            Arc::clone(&font),
            Rc::clone(&cells_rc.floating_width),
            0.0,
            50.0,
            1.0,
            0,
            "Thin bar width (on hover expands to full)",
            Rc::clone(&apply),
        );
    ...
    details.push(
        drag_row(
            Arc::clone(&font),
            Rc::clone(&cells_rc.outer_margin),
            0.0,
            40.0,
            1.0,
            0,
    ...
    // inner-margin control when the bar allocates space).
    {
        ...
        let row = drag_row(
            Arc::clone(&font),
            Rc::clone(&cells_rc.inner_margin),
            0.0,
            40.0,
            1.0,
            0,
    ...
    details.push(
        drag_row(
            Arc::clone(&font),
            Rc::clone(&cells_rc.fade_strength),
            0.0,
            1.0,
            0.05,
            2,
    ...
    col.push(
        wrapped_label(
            Arc::clone(&font),
            "When to show scroll bars; resize the window to see the effect.",
            11.0,
        ),
        0.0,
    );
