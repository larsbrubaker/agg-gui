//! Deterministic frame capture for [`crate::ShellConfig::with_screenshot`].
//!
//! Owns the three pieces the shell needs: the "is this the frame to capture?"
//! predicate, the give-up budget for a capture whose surface never becomes
...
///
/// `painted` is the count of frames painted so far *including* the one just
/// rendered (1-based); `settle_frames` is the caller's requested settle count,
/// clamped to at least 1 so `0` still captures a frame. Only ever true once,
/// so the capture cannot fire twice.
pub(crate) fn should_capture(painted: u64, settle_frames: u32) -> bool {
    painted == settle_frames.max(1) as u64
}

/// How long a pending capture may go without a painted frame before the shell
/// gives up. Generous: the first surface configuration can take a while on a
/// cold start, and a window that is merely being dragged or resized still
/// paints inside this.
...
/// occluded, `Lost`) busy-spins forever: only a painted frame advances the
/// settle count, and the paint path skips whenever `get_current_texture`
/// fails.
...
mod tests {
    ...
    /// burns dozens of redraw attempts in a few milliseconds while the window
    /// server is still configuring the surface, so no plausible attempt count
    /// is safe — only elapsed time is.
    #[test]
    ...
    fn empty_readback_is_an_error() {
        let dir = std::env::temp_dir().join("agg-gui-shell-screenshot-test");
    ...
    fn writes_a_png_and_creates_the_parent_directory() {
        let dir = std::env::temp_dir()
            .join("agg-gui-shell-screenshot-test")
            .join("nested");
