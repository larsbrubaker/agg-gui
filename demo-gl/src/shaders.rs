...
// and gradientTransform, computes SVG's linear/radial gradient parameter,
// applies the spread mode, then samples a 1-D ramp texture built from all stops.
...
#[cfg(target_arch = "wasm32")]
pub(crate) const GRADIENT_FRAG: &str = "#version 300 es\nprecision mediump float;\nin vec2 v_pos;in float v_alpha;uniform sampler2D u_ramp;uniform vec4 u_line;uniform vec4 u_radial;uniform vec2 u_focal;uniform vec4 u_screen_inv_a;uniform vec2 u_screen_inv_b;uniform vec4 u_gradient_inv_a;uniform vec2 u_gradient_inv_b;uniform int u_kind;uniform int u_spread;uniform float u_global_alpha;out vec4 frag_color;vec2 aff(vec4 a,vec2 b,vec2 p){return vec2(p.x*a.x+p.y*a.z+b.x,p.x*a.y+p.y*a.w+b.y);}float spread(float t){if(u_spread==1){return 1.0-abs(mod(t,2.0)-1.0);}if(u_spread==2){return fract(t);}return clamp(t,0.0,1.0);}float linear_t(vec2 p){vec2 a=u_line.xy;vec2 b=u_line.zw;vec2 d=b-a;return dot(p-a,d)/max(dot(d,d),0.000001);}float radial_t(vec2 p){vec2 c=u_radial.xy;float r=max(u_radial.z,0.000001);vec2 f=u_focal;vec2 d=p-f;vec2 fc=f-c;float A=dot(d,d);if(A<=0.000001)return 0.0;float B=2.0*dot(fc,d);float C=dot(fc,fc)-r*r;float disc=max(B*B-4.0*A*C,0.0);float k=(-B+sqrt(disc))/(2.0*A);return (k>0.000001)?(1.0/k):0.0;}void main(){vec2 p=aff(u_screen_inv_a,u_screen_inv_b,v_pos);p=aff(u_gradient_inv_a,u_gradient_inv_b,p);float t=spread((u_kind==1)?radial_t(p):linear_t(p));vec4 c=texture(u_ramp,vec2(t,0.5));frag_color=vec4(c.rgb,c.a*v_alpha*u_global_alpha);}";
#[cfg(not(target_arch = "wasm32"))]
